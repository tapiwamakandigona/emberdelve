...
void main() {
  group('data integrity', () {
    test('twelve provings, unique ids, valid params, encodable', () {
      expect(provings.length, 22); // v0.172.0: the collier's proving
      expect(provings.map((p) => p.id).toSet().length, 22);
  ...
  group('the screen', () {
    testWidgets('lists all eight; locked ones state the requirement', (
      ...
      expect(find.textContaining('0 of 22'), findsOneWidget);
    ...
    testWidgets('cleared proving wears the mark', (tester) async {
      ...
      expect(find.textContaining('1 of 22'), findsOneWidget);
