...
void main() {
  group('data integrity', () {
    test('twelve provings, unique ids, valid params, encodable', () {
      expect(provings.length, 23); // v0.176.0: the stoker's proving
      expect(provings.map((p) => p.id).toSet().length, 23);
  ...
  group('the screen', () {
    testWidgets('lists all eight; locked ones state the requirement', (
      ...
      expect(find.textContaining('0 of 23'), findsOneWidget);
    ...
    testWidgets('cleared proving wears the mark', (tester) async {
      ...
      expect(find.textContaining('1 of 23'), findsOneWidget);
