...
void main() {
  group('data integrity', () {
    test('twelve provings, unique ids, valid params, encodable', () {
      expect(provings.length, 28); // v0.182.0: the lamplighter's proving
      expect(provings.map((p) => p.id).toSet().length, 28);
  ...
  group('the screen', () {
    testWidgets('lists all eight; locked ones state the requirement', (
      ...
      expect(find.textContaining('0 of 28'), findsOneWidget);
    ...
    testWidgets('cleared proving wears the mark', (tester) async {
      ...
      expect(find.textContaining('1 of 28'), findsOneWidget);
