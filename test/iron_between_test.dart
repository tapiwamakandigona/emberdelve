...
void main() {
  ...
  test('elite fights get their own theme; a crowned foe outranks it', () {
    ...
    expect(
      AudioService.musicKeyForPhase('player_turn', eliteFight: true),
      'combat_elite',
    );
    expect(
      AudioService.musicKeyForPhase('player_turn', bossFight: true),
      'boss_combat',
    );
    expect(
    ...
    for (final phase in ['boon', 'map', 'reward', 'shop', 'event', 'rest']) {
      expect(
        AudioService.musicKeyForPhase(phase, eliteFight: true),
        'map',
        reason: '$phase ignores eliteFight',
      );
    }
    expect(
      AudioService.musicKeyForPhase('run_won', eliteFight: true),
      'victory',
    );
    expect(
      AudioService.musicKeyForPhase('run_lost', eliteFight: true),
      'defeat',
    );
    expect(AudioService.musicKeyForPhase(null, eliteFight: true), 'title_menu');
  });
  ...
  test(
    'a run that crosses an elite records the iron as its own record',
    () async {
      ...
      expect(
        c.meta.heardTracks,
        contains('combat_elite'),
        reason: 'the seed-2 road crosses an elite',
      );
      expect(
        c.meta.heardTracks,
        contains('boss_combat'),
