...
class RigPose {
  ...
  static RigPose at(CombatRig rig, RigBeat beat, StrikePlan plan) {
    // Keep the delivered pair's authored poses; the whole-roster extension
    // adds six tool-specific actions sized to each measured native arm.
    if (rig.cutout != null) return _rosterPose(rig, beat, plan);
  ...
  static RigPose _rosterPose(CombatRig rig, RigBeat beat, StrikePlan plan) {
    final w = tierWeight(plan.tier);
    final reach = (rig.elbow - rig.shoulder).distance +
        (rig.wrist - rig.elbow).distance;
    final arm = reach / 10;
    final broad = (rig.frontFoot.dx - rig.rearFoot.dx) / 12;
    final weight = broad.clamp(0.7, 1.25);
    final ready = rig.wrist - rig.shoulder;
    if (beat == RigBeat.ready) {
      return RigPose(handFromShoulder: ready);
    }
    if (beat == RigBeat.guard) {
      return RigPose(
        hips: Offset(-0.35 * weight, 0.8 * weight),
        handFromShoulder: const Offset(3.8, 1.2) * arm,
        chest: 0.045, head: -0.035, spread: 0.65 * weight,
      );
    }
    if (beat == RigBeat.recoil) {
      return RigPose(
        hips: Offset(-0.8 * weight, 0.7),
        handFromShoulder: const Offset(2.5, 3.0) * arm,
        chest: 0.12, head: 0.07, spread: 0.5 * weight,
        cape: 0.025,
      );
    }
    final winding = beat == RigBeat.windup;
    switch (plan.family) {
      case StrikeFamily.cut:
        return winding
            ? RigPose(
                hips: Offset(-0.9 * w, 0.6 * w),
                handFromShoulder: Offset(-1.8 - w, -1.6 - 1.6 * w) * arm,
                chest: 0.055 + 0.095 * w, head: -0.09 * w,
                cape: 0.035 * w, spread: 0.45 * w,
              )
            : RigPose(
                hips: Offset(0.9 * w, -0.3 * w),
                handFromShoulder: Offset(5.8 + w, 1.0 + w) * arm,
                chest: -0.065 - 0.105 * w, head: 0.10 * w,
                cape: -0.06 * w, spread: 0.8 + 0.8 * w,
              );
      case StrikeFamily.crush:
        return winding
            ? RigPose(
                hips: Offset(-0.65 * w, (0.9 + 0.6 * weight) * w),
                handFromShoulder: Offset(-1.0, -3.2 - 2.1 * w) * arm,
                chest: 0.06 + 0.09 * w, head: -0.10 * w,
                spread: 0.6 * w,
              )
            : RigPose(
                hips: Offset(0.6 * w, 0.9 + 0.7 * w * weight),
                handFromShoulder: Offset(5.5 + w, 3.1 + 0.7 * w) * arm,
                chest: -0.06 - 0.09 * w, head: 0.075 * w,
                spread: 0.75 + 0.7 * w,
              );
      case StrikeFamily.stab:
        return winding
            ? RigPose(
                hips: Offset(-0.7 * w, 0.25 * w),
                handFromShoulder: Offset(2.4 - 0.5 * w, 2.2 - 0.6 * w) * arm,
                chest: 0.025 + 0.035 * w, head: -0.035 * w,
                spread: 0.35 * w,
              )
            : RigPose(
                hips: Offset(1.15 * w, -0.25 * w),
                handFromShoulder: Offset(7.8 + 1.4 * w, 0.65) * arm,
                chest: -0.05 - 0.065 * w, head: 0.045 * w,
                spread: 0.8 + 0.8 * w, cape: -0.025 * w,
              );
      case StrikeFamily.stamp:
        return winding
            ? RigPose(
                hips: Offset(-0.35 * w, 0.2 * w),
                handFromShoulder: Offset(2.3, -0.5 - 1.3 * w) * arm,
                chest: 0.018 + 0.035 * w, head: 0.025,
                spread: 0.25 * w,
              )
            : RigPose(
                hips: Offset(0.65 * w, 0.3 * w),
                handFromShoulder: Offset(6.5 + 0.8 * w, 2.3) * arm,
                chest: -0.035 - 0.04 * w, head: 0.045,
                spread: 0.65 + 0.45 * w,
              );
      case StrikeFamily.hook:
        // A tool is offered forward while coiling; contact closes the arm
        // and draws the catch inward rather than imitating a sword swing.
        return winding
            ? RigPose(
                hips: Offset(0.4 * w, 0.35 * w),
                handFromShoulder: Offset(7.8 + 0.6 * w, -0.8 - 0.9 * w) * arm,
                chest: -0.035 - 0.055 * w, head: 0.03 * w,
                spread: 0.4 * w,
              )
            : RigPose(
                hips: Offset(-0.5 * w, 0.65 * w),
                handFromShoulder: Offset(3.8 - 0.5 * w, 3.8 + 0.6 * w) * arm,
                chest: 0.04 + 0.055 * w, head: -0.045 * w,
                spread: 0.8 + 0.6 * w, cape: 0.035 * w,
              );
      case StrikeFamily.pick:
        return winding
            ? RigPose(
                hips: Offset(-0.5 * w, 0.6 * w),
                handFromShoulder: Offset(1.0, -2.4 - 1.4 * w) * arm,
                chest: 0.04 + 0.05 * w, head: -0.07 * w,
                spread: 0.3 * w,
              )
            : RigPose(
                hips: Offset(0.4 * w, 0.7 + 0.4 * w),
                handFromShoulder: Offset(4.8 + 0.6 * w, 3.4) * arm,
                chest: -0.04 - 0.055 * w, head: 0.065 * w,
                spread: 0.6 + 0.5 * w,
              );
    }
  }
