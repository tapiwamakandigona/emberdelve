    ...
    return Stack(
      ...
      children: [
        ...
        Column(
          children: [
            Expanded(
              // PERF (v0.180.0 The Quiet Shell): a LayoutBuilder flushes its
              // dirty descendants during layout, so every rebuild-driven
              // animation inside it (the four SmolderIn verdict lines, the
              // Settling Count) marked it needs-layout every frame — and,
              // with loose width from the Column, that walked 18 levels up
              // to the nearest relayout boundary and repainted the whole
              // shell (summary_probe: ~100–150 paints/frame for the 1.1s
              // entrance, vs 1 at rest). SizedBox.expand makes the
              // LayoutBuilder its own relayout boundary; the RepaintBoundary
              // keeps its per-frame repaint off the vignette, drift and
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      children: [
                                        ...
                                        const Spacer(),
                                        // v0.180.0 The Nearer Fire: 'Delve again' left
                                        // the scroll for the pinned footer below — the
                                        // run-end summary ran 2.25 screens on a 360×800
                                        // phone and the one decision sat 1.4 screens
                                        // down (tool/summary_scroll_visual_test.dart).
            ...
            // v0.180.0 The Nearer Fire: the one decision this screen asks
            // for, pinned where the thumb already is. Fast restart (backlog
            // #8): straight into a new run — boon pick included — without a
            // detour through the title. Same tablet clamp as the column.
            SafeArea(
              top: false,
              child: Center(
                child: ConstrainedBox(
                  ...
                  child: Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: EmberButton(
                        'Delve again',
                        key: const ValueKey('delve-again'),
                        primary: true,
                        icon: Icons.bolt,
                        onTap: () => c.delveAgain(),
                      ),
...
class _SettlingCount extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // PERF (v0.180.0 The Quiet Shell): each digit change used to relayout
    // the whole ledger column — the paragraph's parent chain had no tight
    // box, so `text=` walked up through the IntrinsicHeight column to the
    // viewport and re-rasterized every panel on the screen (~300 paints on
    // each changing frame, summary_probe). The number now lives in a box
    // already the exact size of its final value (tight constraints make
    // the paragraph its own relayout boundary), right-aligned, inside its
    // own repaint boundary: the row never moves, only the digits redraw.
    final painter = TextPainter(
      text: TextSpan(text: '$value', style: style),
      textDirection: TextDirection.ltr,
      textScaler: MediaQuery.textScalerOf(context),
    )..layout();
    final box = painter.size;
    painter.dispose();
