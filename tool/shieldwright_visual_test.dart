// tool/shieldwright_visual_test.dart — manual visual-critique plates for
// v0.158.0 "The Shieldwright": the delver picker scrolled to the shieldwright's
// card (sprite, kit line, price). Not part of CI.
//
//   flutter test tool/shieldwright_visual_test.dart
import 'dart:io';
...
const outDir = 'build/shieldwright_visual';
...
Future<void> capture(
  ...
  await tester.scrollUntilVisible(
    find.text('The Shieldwright'),
    400,
    scrollable: scrollable,
    maxScrolls: 100,
  );
  await tester.pump(const Duration(milliseconds: 100));
  final rowTop = tester.getTopLeft(find.text('The Shieldwright')).dy;
...
void main() {
  ...
  testWidgets('full roster plates', (tester) async {
    ...
    await capture(
      ...
      rosterMeta(),
      const Size(360, 640),
      'shieldwright_card_360x640',
    );
    await capture(
      tester,
      rosterMeta(),
      const Size(320, 568),
      'shieldwright_card_320x568',
    );
  });
}
