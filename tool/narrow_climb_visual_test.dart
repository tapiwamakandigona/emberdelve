...
// tool/narrow_climb_visual_test.dart — manual visual-critique plates for
// v0.76.0 "The New Song". Not part of CI.
//
//   flutter test tool/narrow_climb_visual_test.dart
//
// Plates (build/narrow_climb_visual/):
//   • narrow_climb_360x640 — a first win: the collapsed '"…" and N more join
//     the Gramophone' line sits with the firsts-line, quiet and unclipped.
//   • narrow_climb_320x568 — the restraint plate: narrowest supported width,
...
const outDir = 'build/narrow_climb_visual';
...
Future<void> captureSummary(
  ...
  await tester.scrollUntilVisible(
    find.byKey(const ValueKey('narrow-climb')),
    -200,
...
GameController firstWin() {
  ...
  // Force the narrow ending so the line renders on the plate.
...
void main() {
  ...
  testWidgets('narrow climb plates', (tester) async {
    await loadRealFonts();
    await captureSummary(
      tester,
      firstWin(),
      const Size(360, 640),
      'narrow_climb_360x640',
    );
    await captureSummary(
      tester,
      firstWin(),
      const Size(320, 568),
      'narrow_climb_320x568',
    );
  });
}
