// tool/delvers_window_visual_test.dart — manual visual-critique plates for
// v0.115.0 "The Delver's Window". Not part of CI.
//
//   flutter test tool/delvers_window_visual_test.dart
//
// Plates (build/delvers_window_visual/):
//   • window_shelf_360x640 — THE VISTA shelf dressing the WARDEN via the
//     new dress pills: Moonveil CHOSEN for the warden only.
//   • window_map_360x800 — a warden run wearing the warden's own window
//     (deepshale) while the global selection stays emberlight.
//   • window_shelf_320x568 — restraint plate at the narrowest width.
import 'dart:io';
...
const outDir = 'build/delvers_window_visual';
...
Future<void> captureShelf(
  ...
  await tester.scrollUntilVisible(
    find.byKey(const ValueKey('vista-dress-warden')),
    400,
    scrollable: find.byType(Scrollable).first,
  );
  await tester.tap(find.byKey(const ValueKey('vista-dress-warden')));
  await tester.pump(const Duration(milliseconds: 200));
  await tester.pump(const Duration(milliseconds: 100));
  final top = tester
      .getTopLeft(find.byKey(const ValueKey('vista-dress-warden')))
      .dy;
  await tester.drag(
...
void main() {
  ...
  testWidgets('delvers window plates', (tester) async {
    ...
    await captureShelf(
      ...
      shelfMeta(),
      const Size(360, 640),
      'window_shelf_360x640',
    );
    await captureShelf(
      tester,
      shelfMeta(),
      const Size(320, 568),
      'window_shelf_320x568',
    );
  });
}
