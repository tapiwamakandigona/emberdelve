// tool/hearthkeeper_visual_test.dart — manual visual-critique plates for
// v0.175.0 "The Stoker": the delver picker scrolled to the hearthkeeper's
// card (sprite, kit line, price). Not part of CI.
//
//   flutter test tool/hearthkeeper_visual_test.dart
import 'dart:io';
...
const outDir = 'build/hearthkeeper_visual';
...
Future<void> capture(
  ...
  await tester.scrollUntilVisible(
    find.text('The Stoker'),
    400,
    scrollable: scrollable,
    maxScrolls: 100,
  );
  await tester.pump(const Duration(milliseconds: 100));
  final rowTop = tester.getTopLeft(find.text('The Stoker')).dy;
...
void main() {
  ...
  testWidgets('full roster plates', (tester) async {
    ...
    await capture(
      ...
      rosterMeta(),
      const Size(360, 640),
      'hearthkeeper_card_360x640',
    );
    await capture(
      tester,
      rosterMeta(),
      const Size(320, 568),
      'hearthkeeper_card_320x568',
    );
  });
}
