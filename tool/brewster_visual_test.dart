...
// tool/brewster_visual_test.dart — manual visual-critique plates for THE
// HEDGER (v0.179.0): character screen with the seventeenth chair unlocked,
// and the brewster standing in combat. Not part of CI.
//
//   flutter test tool/brewster_visual_test.dart
import 'dart:io';
...
const outDir = 'build/brewster_visual';
...
void main() {
  testWidgets('character screen with the brewster (360x640 @1.0)', (
    tester,
  ) async {
    await loadRealFonts();
    ...
    // The brewster is the LAST row — drag to the bottom in big steps
    ...
    await shoot(tester, key, 'character-screen-brewster');
  });
  ...
  testWidgets('brewster stands in combat (seed 29 normal)', (tester) async {
    await loadRealFonts();
    tester.view.physicalSize = const Size(360 * 3, 640 * 3);
    tester.view.devicePixelRatio = 3.0;
    addTearDown(tester.view.reset);
    final c = seasoned();
    c.startRun(character: 'brewster', seed: 29, boons: false);
    var guard = 0;
    ...
    await shoot(tester, key, 'combat-brewster');
  });
}
