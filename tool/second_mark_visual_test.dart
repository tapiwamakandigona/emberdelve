// tool/second_mark_visual_test.dart — manual visual-critique plates for
// v0.132.0 "The Second Mark": the rest screen's counted temper copy in
// both states. Not part of CI.
//
//   flutter test tool/second_mark_visual_test.dart
//
// Plates (build/second_mark_visual/):
//   • fresh-phone / fresh-small — 'Two marks a delve.'
//   • spent1-phone — 'One mark left.' after the first temper.
import 'dart:io';
...
const outDir = 'build/second_mark_visual';
...
void main() {
  ...
  testWidgets('one mark spent @ phone', (tester) async {
    await tester.binding.runAsync(loadRealFonts);
    tester.view.physicalSize = const Size(412, 915) * 3;
    tester.view.devicePixelRatio = 3;
    addTearDown(tester.view.reset);
    final c = _atRest();
    c.apply({'type': 'temper_face', 'die': 1, 'face': 4, 'rune': 'gilt'});
    c.sim!.phase = 'rest';
    final key = await _pumpRest(tester, c);
    await _shoot(tester, key, 'spent1-phone');
  });
}
