...
// tool/lamplighter_visual_test.dart — manual visual-critique plates for THE
// HEDGER (v0.179.0): character screen with the seventeenth chair unlocked,
// and the lamplighter standing in combat. Not part of CI.
//
//   flutter test tool/lamplighter_visual_test.dart
import 'dart:io';
...
const outDir = 'build/lamplighter_visual';
...
void main() {
  testWidgets('character screen with the lamplighter (360x640 @1.0)', (
    tester,
  ) async {
    await loadRealFonts();
    ...
    // The lamplighter is the LAST row — drag to the bottom in big steps
    ...
    await shoot(tester, key, 'character-screen-lamplighter');
  });
  ...
  testWidgets('lamplighter stands in combat (seed 20 normal)', (tester) async {
    ...
    addTearDown(tester.view.reset);
    final c = seasoned();
    c.startRun(character: 'lamplighter', seed: 20, boons: false);
    var guard = 0;
    ...
    await shoot(tester, key, 'combat-lamplighter');
  });
}
