// tool/runesmith_visual_test.dart — manual visual-critique plates for
// v0.135.0 "The Runesmith": the locked eighth-delver card in the picker
// at two widths (sprite, price, card text). Warmup capture first (sprite
// decode cache lesson). Not part of CI.
//
//   flutter test tool/runesmith_visual_test.dart
import 'dart:io';
...
const outDir = 'build/runesmith_visual';
...
Future<void> capture(
  ...
  await tester.scrollUntilVisible(
    find.text('The Runesmith'),
    400,
    scrollable: scrollable,
    maxScrolls: 100,
  );
  await tester.pump(const Duration(milliseconds: 100));
  final rowTop = tester.getTopLeft(find.text('The Runesmith')).dy;
...
void main() {
  ...
  testWidgets('full roster plates', (tester) async {
    ...
    await capture(
      ...
      rosterMeta(),
      const Size(360, 640),
      'runesmith_card_360x640',
    );
    await capture(
      tester,
      rosterMeta(),
      const Size(320, 568),
      'runesmith_card_320x568',
    );
  });
}
