...
// tool/glover_visual_test.dart — manual visual-critique plates for THE
// HEDGER (v0.179.0): character screen with the seventeenth chair unlocked,
// and the glover standing in combat. Not part of CI.
//
//   flutter test tool/glover_visual_test.dart
import 'dart:io';
...
const outDir = 'build/glover_visual';
...
void main() {
  testWidgets('character screen with the glover (360x640 @1.0)', (
    tester,
  ) async {
    await loadRealFonts();
    ...
    // The glover is the LAST row — drag to the bottom in big steps
    ...
    await shoot(tester, key, 'character-screen-glover');
  });
  ...
  testWidgets('SEAM the second-circle header (360x640 @1.0)', (tester) async {
    await loadRealFonts();
    tester.view.physicalSize = const Size(360 * 3, 640 * 3);
    tester.view.devicePixelRatio = 3.0;
    addTearDown(tester.view.reset);
  ...
  testWidgets('glover stands in combat (seed 22 normal)', (tester) async {
    await loadRealFonts();
    tester.view.physicalSize = const Size(360 * 3, 640 * 3);
    tester.view.devicePixelRatio = 3.0;
    addTearDown(tester.view.reset);
    final c = seasoned();
    c.startRun(character: 'glover', seed: 22, boons: false);
    var guard = 0;
    ...
    await shoot(tester, key, 'combat-glover');
  });
}
