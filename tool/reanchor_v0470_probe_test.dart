// tool/reanchor_v0470_probe_test.dart — one-shot re-anchor probe for the
// v0.47.0 response-puzzle drop (enemies 39->42). Not part of CI.
// Prints everything the sixth golden re-anchor needs:
//   1. goldenV6 replacement (seed 20260723 full-run hash).
//   2. Boss reach per residue class + new bossGoldens.
//   3. Kindler easy-loss seed (boons on) to replace the current one.
//   4. Peddler pins: seed 1 easy/normal verify + easy-loss seed to replace the current one.
//   5. Peddler normal-win set among seeds 1..8 (docs pin {1,2,4,6,8}).
//   6. ash_summit proving: ascetic hard A15 winnable seed to replace 912.
...
void main() {
  ...
  test('ash_summit re-hunt', () {
    // Old seed 912; try it first, then hunt forward.
    for (var s = 912; s <= 1400; s++) {
