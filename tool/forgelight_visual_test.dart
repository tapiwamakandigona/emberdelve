// tool/forgelight_visual_test.dart — manual visual-critique plates for
// v0.126.0 "The Forgelight". Not part of CI.
//
//   flutter test tool/forgelight_visual_test.dart
//
// Plates (build/forgelight_visual/):
//   • forgelight_map_360x800 — the map under the forgelight grade.
//   • emberlight_map_360x800 — same seed, identity vista (control).
//   • forgelight_card_locked_320x568 — the vista shelf scrolled to the
//     locked card (unlock line visible).
//   • forgelight_card_worn_360x640 — unlocked and worn.
import 'dart:io';
...
const outDir = 'build/forgelight_visual';
...
GameController shelfMeta({required bool unlocked}) {
  ...
  c.meta
    ..runsWon = 3
    ..bestFloor = 6
    ..tempersSet = unlocked ? 10 : 4;
  if (unlocked) c.selectVista('forgelight');
  return c;
}
...
Future<void> captureShelf(
  ...
  await tester.scrollUntilVisible(
    find.byKey(const ValueKey('vista-forgelight')),
    400,
    scrollable: find.byType(Scrollable).first,
  );
  await tester.pump(const Duration(milliseconds: 100));
  final top = tester
      .getTopLeft(find.byKey(const ValueKey('vista-forgelight')))
      .dy;
  await tester.drag(
...
void main() {
  ...
  testWidgets('frostvein plates', (tester) async {
    ...
    await captureShelf(
      ...
      const Size(320, 568),
      'forgelight_card_locked_320x568',
    );
    await captureShelf(
      tester,
      shelfMeta(unlocked: true),
      const Size(360, 640),
      'forgelight_card_worn_360x640',
    );
  });
}
